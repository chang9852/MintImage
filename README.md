...
所以模型名要用中转站自己的命名，可点模型名输入框右侧的云朵图标获取模型列表。

中转站的 Grok 渠道只接受一组固定的尺寸标记（`1024x1024`、`1280x720`、
`720x1280`、`1536x1024`、`1024x1536`），其他像素尺寸会被直接拒绝
（报错「aspect_ratio 不受支持」）。应用会把所选尺寸换算到其中比例最接近的一档，
选「自动」则不下发尺寸、交由上游决定。
...
Grok Imagine 的每次生成都会**先经过一遍提示词重写模型**（官方说明这是强制开启、
无法关闭），提示词越长、越复杂，这一步耗时就越久，再加上上游排队，
单项请求常常需要数分钟。

- 应用对 Grok 模型设有 1200 秒的超时下限，即便设置里的超时更短也按此执行，
  避免把仍在生成中的请求提前中断；设置里填更大的值则以设置为准。
- 想更快出图，最有效的是**把提示词写短**。Grok 会自动补全细节，
  堆叠「8K 画质 / 电影级质感 / 85mm 定焦」这类修饰词反而更慢。
